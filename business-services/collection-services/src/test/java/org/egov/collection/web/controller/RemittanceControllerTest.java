...
class RemittanceControllerTest {
    ...
    /**
     * Method under test: {@link RemittanceController#create(RemittanceRequest)}
     */
    @Test
    void testCreate() throws Exception {
        when(remittanceService.createRemittance((RemittanceRequest) any())).thenReturn(new Remittance());
        ...
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders.post("/remittances/_create")
                ...
                .content(content);
        MockMvcBuilders.standaloneSetup(remittanceController)
                .build()
    ...
    void testCreate2() throws Exception {
        ...
        RemittanceRequest remittanceRequest = new RemittanceRequest();
        remittanceRequest.setRemittances(remittanceList);
        remittanceRequest.setRequestInfo(new RequestInfo());
        String content = (new ObjectMapper()).writeValueAsString(remittanceRequest);
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders.post("/remittances/_create")
                .contentType(MediaType.APPLICATION_JSON)
                .content(content);
        ResultActions actualPerformResult = MockMvcBuilders.standaloneSetup(remittanceController)
                .build()
                .perform(requestBuilder);
    ...
    /**
     * Method under test: {@link RemittanceController#search(RemittanceSearchRequest, RequestInfoWrapper)}
     */
    @Test
    void testSearch() throws Exception {
        when(remittanceService.getRemittances((RequestInfo) any(), (RemittanceSearchRequest) any()))
    ...
    /**
     * Method under test: {@link RemittanceController#update(RemittanceRequest)}
     */
    @Test
    void testUpdate() throws Exception {
        when(remittanceService.updateRemittance((RemittanceRequest) any())).thenReturn(new Remittance());
