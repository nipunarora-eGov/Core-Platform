...
class PreExistPaymentControllerTest {
    ...
    /**
     * Method under test: {@link PreExistPaymentController#update()}
     */
    @Test
    void testUpdate() throws Exception {
        when(paymentRepository.fetchIfsccode()).thenReturn(new ArrayList<>());
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders.post("/preexistpayments/_update");
        MockMvcBuilders.standaloneSetup(preExistPaymentController)
                .build()
    ...
    void testUpdate2() throws Exception {
        ...
        stringList.add("?");
        when(paymentRepository.fetchIfsccode()).thenReturn(stringList);
        doNothing().when(preExistPaymentService).updatePaymentBankDetails((String) any());
        MockHttpServletRequestBuilder requestBuilder = MockMvcRequestBuilders.post("/preexistpayments/_update");
        MockMvcBuilders.standaloneSetup(preExistPaymentController)
                .build()
                .perform(requestBuilder)
