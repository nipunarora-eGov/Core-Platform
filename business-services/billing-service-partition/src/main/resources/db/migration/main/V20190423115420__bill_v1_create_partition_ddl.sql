...
(
    ...
    CONSTRAINT pk_egbs_bill_v1 PRIMARY KEY (id, tenantid ,status)
)PARTITION BY LIST (status);

CREATE TABLE status_active PARTITION OF egbs_bill_v1 FOR VALUES IN ('ACTIVE');
CREATE TABLE status_expired PARTITION OF egbs_bill_v1 FOR VALUES IN ('EXPIRED');
...
CREATE INDEX IF NOT EXISTS idx_egbs_bill_v1_status ON egbs_bill_v1(status);
...
(
    ...
    expirydate bigint,
    status character varying(64),
    CONSTRAINT pk_egbs_billdetail_v1 PRIMARY KEY (id, tenantid,businessservice),
    CONSTRAINT fk_egbs_bill_v1 FOREIGN KEY (tenantid, billid ,status) REFERENCES egbs_bill_v1 (tenantid, id ,status)
)PARTITION BY LIST (businessservice);

 CREATE TABLE PT PARTITION OF egbs_billdetail_v1 FOR VALUES IN ('PT');
 CREATE TABLE WS PARTITION OF egbs_billdetail_v1 FOR VALUES IN ('WS');
...
CREATE INDEX IF NOT EXISTS idx_egbs_billdetail_v1_status ON egbs_billdetail_v1(status);
...
(
    ...
    businessservice character varying(250) ,
    CONSTRAINT pk_egbs_billaccountdetails_v1 PRIMARY KEY (id, tenantid),
    CONSTRAINT fk_egbs_billdetail_v1 FOREIGN KEY (billdetail, tenantid,businessservice) REFERENCES egbs_billdetail_v1 (id, tenantid,businessservice)
);